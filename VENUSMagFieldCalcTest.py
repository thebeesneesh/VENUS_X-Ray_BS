...
class VenusApp(tk.Tk):
    ...
    def _calculate_axial(self):
        try:
            ...
            sextupole = float(self.sextupole_entry.get())
            ...
            self.axial_axes[0].set_title("Total Axial Magnetic Field")
            self.axial_axes[0].set_xlabel("Z (cm)")
            self.axial_axes[0].set_ylabel("Bz (T)")
            self.axial_axes[0].grid(True)
            ...
            self.axial_axes[1].grid(True)
            self.axial_axes[1].legend()
...
def run_program():
    ...
    b_res = read_float("Bres in tesla: ")
    ...
    # Find injection and extraction peaks.
    injection_mask = (calculator.z_cm >= -100.0) & (
        calculator.z_cm <= 0.0
    )
    extraction_mask = (calculator.z_cm >= 0.0) & (
        calculator.z_cm <= 100.0
    )

    injection_indices = np.where(injection_mask)[0]
    extraction_indices = np.where(extraction_mask)[0]

    injection_peak_index = injection_indices[
        np.argmax(bz[injection_indices])
    ]
    extraction_peak_index = extraction_indices[
        np.argmax(bz[extraction_indices])
    ]

    zmin = z[injection_peak_index]
    zmax = z[extraction_peak_index]

    dbz_dz, bz_second = make_derivatives(z, bz)
    dbz_dz_second = spline(z, dbz_dz)

    # Six source bars at 10 m.
    bars = np.zeros((2, 6))
    ...
    # The known radial sextupole field is always evaluated at 7 cm.
    known_radius_cm = 7.0
    known_radius_m = known_radius_cm / 100.0
    ...
    # Fixed search limits and mesh resolution.
    rmax = 7.2 / 100.0
    nx = ny = nz = 100
    ...
    z_point_min = float("inf")
    z_point_max = float("-inf")
    radius_point_max = 0.0
    ...
    for i in range(nx):
        for j in range(ny):
            for k in range(nz):
                ...
                bx = sext_field[0] + solenoid_field[0]
                by = sext_field[1] + solenoid_field[1]
                bz_value = solenoid_field[2]

                total_field = sqrt(
                    bx**2 + by**2 + bz_value**2
                )

                if total_field <= b_res:
                    total_volume += cell_volume
                    z_point_min = min(z_point_min, z_position)
                    z_point_max = max(z_point_max, z_position)
                    radius_point_max = max(
                        radius_point_max,
                        radius,
                    )

    print(
        "with dx[cm]=%.4f, dy[cm]=%.4f, dz[cm]=%.4f "
        "dvol[cm^3]=%.4f"
        % (
            dx * 100.0,
            dy * 100.0,
    ...
    print("total Volume[cm^3]=%.2f" % (total_volume * 1.0e6))

    print(
        "\ndefined: zmin[cm]=%.2f, zmax[cm]=%.2f, "
        "rmax[cm]=%.2f"
        % (
            zmin * 100.0,
            zmax * 100.0,
    ...
    print(
        "actual points: zmin[cm]=%.2f, zmax[cm]=%.2f, "
        "rmax[cm]=%.2f"
        % (
            z_point_min * 100.0,
            z_point_max * 100.0,
            radius_point_max * 100.0,
        )
    )
