...
def RunProgram():
    ...
    Bres = float(input("Bresonance in tesla: "))
        ...
        for i in range(nx):
            for j in range(ny):
                for k in range(nz):
                    ...
                    if rpt < rmax:
                        Bsext = GetSextField(xpt, ypt, mult, bars)
                        Bsol = GetSolenoidField(xpt, ypt, zpt, rpt, z, dzmesh, Bz, Bz2, dBz_dz, dBz_dz2)
                        Btot[0] = Bsext[0] + Bsol[0]
                        Btot[1] = Bsext[1] + Bsol[1]
                        Btot[2] = Bsol[2]
...
def GetNecessaryArrays():
    z, Bz = GetAxialField()
    dBz_dz, Bz2 = makeDerivatives(z, Bz)
    dBz_dz2 = spline(z, dBz_dz)
...
def GetSextField(xpt, ypt, mult, bars):
...
def GetSolenoidField(xpt, ypt, zpt, rpt, z, dz, Bz, Bz2, dBz_dz, dBz_dz2):
...
def makeDerivatives(z, Bz):
    dBz_dz = np.zeros_like(Bz)
    d2Bz_dz2 = spline(z, Bz)
    dz = z[1] - z[0]
    dz_sm = 0.001 * dz
...
def GetAxialField():
    ...
    Nheader = len(header_lines)
    nlines = len(lines)

    Bz = np.zeros(nlines - Nheader - 2, float)
    z = np.zeros(nlines - Nheader - 2, float)
